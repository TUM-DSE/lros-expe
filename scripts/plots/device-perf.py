...
data = pd.concat([
    ...
    generate_input_data(("vAccel", 3, 1.5), ["Llama3.1-1B", "Qwen2.5-1.5B", "Gemma3-1B"],
                        [[6.4, 7.4, 8.1, 8.7, 15.8, 24.5, 28.0, 31.0, 39.1, 70.1],
                         [9.1, 10.9, 11.8, 12.5, 22.9, 34.7, 39.4, 43.0, 55.1, 100.4],
                         [4.8, 5.6, 6.0, 6.5, 11.8, 18.1, 21.2, 23.1, 30.0, 54.5]], [3.2, 2.8, 4.3]),
    generate_input_data(("Native", 3.5, 1.7), ["Llama3.1-1B", "Qwen2.5-1.5B", "Gemma3-1B"],
                        [[6.4, 7.4, 8.1, 8.7, 15.8, 24.5, 28.0, 31.0, 39.1, 70.1],
                         [9.1, 10.9, 11.8, 12.5, 22.9, 34.7, 39.4, 43.0, 55.1, 100.4],
                         [4.8, 5.6, 6.0, 6.5, 11.8, 18.1, 21.2, 23.1, 30.0, 54.5]], [3.2, 2.8, 4.3]),
])
...
for m, x in zip(["Llama3.1-1B", "Gemma3-1B", "Qwen2.5-1.5B"], string.ascii_lowercase):
    ...
    fig.savefig(os.path.join(plots_dir, "mock", f"device-perf-ttft-{m}.pdf"))
...
ax.legend(title=None)
fig.tight_layout(pad=0.1)
fig.savefig(os.path.join(plots_dir, "mock", "device-perf-ttft.pdf"))

fig, ax = plt.subplots(figsize=(figwidth_full, fig_height))
sns.lineplot(data=data, x="output_token_count", y="TPOT", ax=ax, hue="type", style="type", markers=True,
             dashes=False, palette=palette[:3])
...
ax.legend(title=None)
fig.tight_layout(pad=0.1)
fig.savefig(os.path.join(plots_dir, "mock", "device-perf-tpot.pdf"))

fig, ax = plt.subplots(figsize=(figwidth_half, fig_height))
sns.barplot(data=data, x="Model", y="TPOT", ax=ax, hue="type", palette=palette[:3])
#ax.set_ylim(bottom=0, top=0.4)
ax.set_ylabel("TPOT [s]")
ax.set_xlabel(None)
ax.set_title(lower_better_str, fontsize=FONTSIZE, color="navy")
...
fig.tight_layout(pad=0.1)
fig.tight_layout(pad=0.1)
fig.savefig(os.path.join(plots_dir, "mock", "device-perf-tpot1.pdf")
